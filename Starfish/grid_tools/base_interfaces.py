...
class NPZInterface:
    """
    Connect to an NPZ file that stores spectra.

    Parameters
    ----------
    filename : str or path-like
        The path of the saved NPZ file
    """

    def __init__(self, filename):
        self.filename = os.path.expandvars(filename)

        # In order to properly interface with the NPZ file, we need to learn
        ...
        self.wl = data["wl"]
        self.key_name = str(data["flux_key_name"])
        self.wl_header = data["wl_header"].item()
        self.dv = self.wl_header["dv"]
        self.grid_points = data["grid_points"]
        self.param_names = data["param_names"]
        self.wave_units = str(data["wave_units"])
        self.flux_units = str(data["flux_units"])
        self.flux_data = data["flux_data"].item()  # Dictionary of flux arrays
    ...
    def load_flux(self, parameters, header=False):
        ...
        flux_entry = self.flux_data[key]
        fl = flux_entry["flux"]
        hdr = flux_entry.get("header", {})
        
        if self.ind is not None:
            fl = fl[self.ind[0] : self.ind[1]]
...
class NPZCreator:
    """
    Create a NPZ grid to store all of the spectra from a RawGridInterface,
    along with metadata.

    Parameters
    ----------
    grid_interface : :class:`GridInterface`
        The raw grid interface to process while creating the NPZ file
    filename : str or path-like
    ...
    def __init__(
        ...
        # Initialize storage for NPZ data
        self.grid_name = grid_interface.name
        self.flux_units = grid_interface.flux_units
        self.flux_data = {}  # Dictionary to store flux data
        ...
        # Store wavelength information for NPZ
        self.wl_final_data = self.wl_final.astype(np.float64)  # Use float64 due to rounding errors w/ interpolation
        self.wl_header_data = {
            "air": self.grid_interface.air,
            "dv": self.dv_final,
            "units": self.grid_interface.wave_units
        }
    ...
    def process_grid(self):
        ...
        for i, param in enumerate(pbar):
            #pbar.set_description("Processing Grid: {}".format(param)) # add description to progress bar
...
HDF5Interface = NPZInterface
HDF5Creator = NPZCreator
